...
import { works, publications, commissions } from '../src/data/content.js';
...
if (process.argv[1] && import.meta.url === pathToFileURL(resolve(process.argv[1])).href) {
  ...
  const result = validateGalleryImages({ works, books: publications, commission: commissions }, imagesRoot);
  if (result.errors.length) {
    for (const error of result.errors) console.error(error);
    process.exitCode = 1;
  } else {
    console.log(`Validated ${result.checked} gallery image references.`);
  }
}
